...
import com.ajpr00.tablet.presentation.transitions.transitionFor
...
fun Reproductor(viewModel: ReproducorViewModel) {
    ...
    val blurModifier =
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) Modifier.fillMaxSize().blur(50.dp) else Modifier
            .fillMaxSize()
            .graphicsLayer {
                alpha = 0.25f
                scaleX = 1.5f
        ...
        when (current?.type) {
            ...
            FormatType.IMAGE -> {
                ...
                AsyncImage(
                    ...
                    modifier = Modifier
                        .fillMaxSize(),
                    contentScale = ContentScale.Fit  // Mantiene proporción inicial
                )
            }
